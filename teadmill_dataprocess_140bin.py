...
LHEE_RHS = LHEE[RHS_indx[0:420],:]
RHEE_RHS = RHEE[RHS_indx[0:420],:]
StpLen_R = abs(np.subtract(RHEE_RHS[:,0],LHEE_RHS[:,0])) #right step length


RHEE_LHS = RHEE[LHS_indx[0:420],:]
LHEE_LHS = LHEE[LHS_indx[0:420],:]
StpLen_L = abs(np.subtract(LHEE_LHS[:,0],RHEE_LHS[:,0])) #left step length


RLHL_RHS = RLHL[RHS_indx[0:420],:]
LLHL_RHS = LLHL[RHS_indx[0:420],:]
Stp_WidR = abs(np.subtract(RLHL_RHS[:,2],LLHL_RHS[:,2])) #right step widths


RLHL_LHS = RLHL[LHS_indx[0:420],:]
LLHL_LHS = LLHL[LHS_indx[0:420],:]
Stp_WidL = abs(np.subtract(RLHL_LHS[:,2],LLHL_LHS[:,2])) #left step widths




Bin_Num = int(len(RHS_indx)/140) #number of bins based on 25 strides each
RHS_len = len(RHS_indx)

test_indx = np.zeros((140, Bin_Num))
for i in range(Bin_Num):
    test_indx[:, i] = RHS_indx[i*140:(i+1)*140]



MOS_bin = int(len(MOS_ML_R[:420])/140)
MOS_ML_Rgt = np.zeros((140, MOS_bin))
MOS_ML_Lft = np.zeros((140, MOS_bin))
MOS_AP_Rgt = np.zeros((140, MOS_bin))
MOS_AP_Lft = np.zeros((140, MOS_bin))


for i in range(MOS_bin):
    MOS_ML_Rgt[:, i] = MOS_ML_R[i*140:(i+1)*140]
    MOS_ML_Lft[:, i] = MOS_ML_L[i*140:(i+1)*140]
    MOS_AP_Rgt[:, i] = MOS_AP_R[i*140:(i+1)*140]
    MOS_AP_Lft[:, i] = MOS_AP_L[i*140:(i+1)*140]


RHS_bin = int(len(RHS_indx[:420])/140)
RHS_ind = np.zeros((140, RHS_bin))
#RTO_ind = np.zeros((25, RHS_bin))


LHS_bin = int(len(LHS_indx[:420])/140)
LHS_ind = np.zeros((140, LHS_bin))
#LTO_ind = np.zeros((25, LHS_bin))

Stp_WidR = Stp_WidR[0:420]
Stp_WidL = Stp_WidL[0:420]
StpLen_R = StpLen_R[0:420]
StpLen_L = StpLen_L[0:420]
Step_W_R = np.zeros((140, LHS_bin))
Step_W_L = np.zeros((140, LHS_bin))
Step_LN_R = np.zeros((140, LHS_bin))
Step_LN_L = np.zeros((140, LHS_bin))



StpT_R = np.zeros((140, LHS_bin))
StpT_L =  np.zeros((140, LHS_bin))

RStep_Times = RStep_Times[:420]
LStep_Times = LStep_Times[:420]

for i in range(RHS_bin):
    StpT_R[:,i] = RStep_Times[i*140:(i+1)*140]
    RHS_ind[:, i] = RHS_indx[i*140:(i+1)*140]
   # RTO_ind[:, i] = RTO_indx[i*25:(i+1)*25]
    StpT_L[:,i] = LStep_Times[i*140:(i+1)*140]
    LHS_ind[:, i] = LHS_indx[i*140:(i+1)*140]
    #LTO_ind[:, i] = LTO_indx[i*25:(i+1)*25]
    Step_W_R[:,i] = Stp_WidR[i*140:(i+1)*140]
    Step_W_L[:,i] = Stp_WidL[i*140:(i+1)*140]
    Step_LN_R[:,i] = StpLen_R[i*140:(i+1)*140]
    Step_LN_L[:,i] = StpLen_L[i*140:(i+1)*140]
